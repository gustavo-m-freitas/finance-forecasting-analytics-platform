...
# SQL query (Income Statement - annual view)
sql_query = """
    SELECT *
    FROM finance_ifrs_dw.vw_pl_annual
    ORDER BY year
"""
df_pl = pd.read_sql(sql_query, engine)
...
# SQL query (Cash Flow - annual view)
sql_query_cf = """
    SELECT *
    FROM finance_ifrs_dw.vw_cf_annual
    ORDER BY year
"""
df_cf = pd.read_sql(sql_query_cf, engine)
...
csv_path = PROJECT_ROOT / "Module_3" / "py_files_cvs" / "ifrs_financial_indicators_T.csv"
...
# =========================================================
    ...
    sql_sales = """
        ...
        FROM finance_ifrs_dw.fact_sales s
        JOIN finance_ifrs_dw.dim_date    dd ON s.datekey    = dd.datekey
        JOIN finance_ifrs_dw.dim_channel dc ON s.channelkey = dc.channelkey
        JOIN finance_ifrs_dw.dim_product dp ON s.productkey = dp.productkey
        WHERE s.scenariokey = 1
        GROUP BY dd.year, dc.channelname, dp.productname
        ORDER BY dd.year
    """
    df_sales_raw = pd.read_sql(sql_sales, engine)
    # fact_sales.amount is stored in thousands → convert to actual units
    # so all revenue values are on the same scale as price_volume (price × units)
    df_sales_raw["revenue"] = df_sales_raw["revenue"] * 1000

    sql_pv = """
        SELECT
            dd.year,
            SUM(pv.units) AS total_units,
            SUM(pv.price * pv.units) / NULLIF(SUM(pv.units), 0) AS avg_price
        FROM finance_ifrs_dw.fact_price_volume pv
        JOIN finance_ifrs_dw.dim_date dd ON pv.datekey = dd.datekey
        GROUP BY dd.year
        ORDER BY dd.year
    """
    df_pv_raw = pd.read_sql(sql_pv, engine)
...
# =========================================================
    ...
    sql_act = """
        SELECT *
        FROM finance_ifrs_dw.vw_pl_monthly
        WHERE year::int IN (2023, 2024)
        ORDER BY year, month
    """
    ...
    sql_fcast = """
        SELECT
            ...
            revenue,
            ebitda,
            ebitda / NULLIF(revenue, 0) * 100 AS ebitda_pct,
            net_income,
            net_income / NULLIF(revenue, 0) * 100 AS net_margin_pct
        FROM finance_ifrs_dw.vw_pl_monthly_forecast
        ORDER BY year, month
    """
    df_fcast = pd.read_sql(sql_fcast, engine)

    # Scenario CSV
    FC_CSV = Path(__file__).resolve().parent.parent.parent / "final_23_forecast.csv"
    df_sc = pd.read_csv(FC_CSV)
